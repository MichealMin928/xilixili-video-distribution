...
洗哩洗哩视频分发是一个完全在本机运行的抖音、小红书、快手、微信视频号统一发布工具。它不依赖第三方聚合账号，使用专用的真实 Google Chrome 登录态，并提供浏览器界面、HTTP API 和 Codex 可调用的 CLI。
...
- 内容选片台自动读取 `content/` 中素材完整的清单，展示真实封面，并安全创建任务、预填平台；`archive/` 中的历史版本不会进入当前内容池。
- 创建一份基础内容并自动生成四个平台的长度与话题格式版本。
- 按江西酒店决策者的工作节奏生成每日四平台错峰排期，并随任务保存。
- 视频或图文素材依次预填到四个平台，默认停在发布按钮前。
- 重试只补做失败或未完成的平台，已成功的平台页面和结果不会被刷新、重传或覆盖。
- 小红书话题使用平台原生候选选择；视频号会核对完整正文；立即发布会先确认平台处于“立即”模式。
- 视频任务必须先确认无平台/AI 水印、网址和二维码；公开文案会拦截内部制作说明式旁白。
...
`start` 可重复执行，只会补做尚未成功预填的平台；已经成功的平台不会刷新页面或重新上传。相同标题和素材默认不允许重复发布。`publish` 会阻止未预填、已发布或尚未到计划时间的操作。
...
4. 视频必须先确认不含平台水印、AI 生成水印、网址或二维码；不把内部制作说明当成公开文案发布。
5. 删除作品、修改账号资料和私信回复不纳入批量发布接口。
6. 平台页面改版后，适配器应先在“预填”阶段验证，不能直接试发。
7. `xilixili-service uninstall` 只移除系统服务和命令，默认保留 `.local/` 中的账号资料、任务和日志。
